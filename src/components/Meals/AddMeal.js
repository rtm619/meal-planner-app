...
import Schema from './AddMeal.schema'
import Button from '../Button/Button'
import constants from '../../constants'
import styles from './styles/AddMeal.style'
...
class AddMeal extends Component {
  ...
  constructor(props) {
    ...
    Object.keys(Schema).forEach(item => {
      if (Schema[item].type === 'date') {
        this.state[item] = date.toJSON().substring(0, 10)
      } else {
        this.state[item] = ''
  ...
  handleSubmit = async () => {
    const { mealStore, authStore, handleClose, history } = this.props
    const errors = []
    ...
    if (errors.length === 0) {
      ...
      const responseStatus = await mealStore.add({ name: this.state.name, date: date.toJSON(), calories: this.state.calories })
      handleClose()
...
export default inject('mealStore', 'authStore')(observer(AddMeal))
